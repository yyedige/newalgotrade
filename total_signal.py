# ============================================
# Ensemble Signal - Fixed for Pipeline (with LSTM direction)
# ============================================
...
lstm_df  = pd.read_csv(signals_dir / "lstm_direction_signal.csv", parse_dates=["Date"])
...
ensemble = ensemble.merge(
    arima_df[["Date", "signal"]].rename(columns={"signal": "signalARIMA"}),
    on="Date",
    how="outer",
)
ensemble = ensemble.merge(
    lstm_df[["Date", "signalLSTM"]],
    on="Date",
    how="outer",
)
...
metrics.append(calculate_metrics(ensemble["ret_lstm"], "LSTM Direction"))
